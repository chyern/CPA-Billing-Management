...
func usageEventFromRecord(record UsageRecord, cost float64, pricedBy string, priced bool) UsageEvent {
	return UsageEvent{Domain: record.Domain, RequestedAt: record.RequestedAt, Provider: record.Provider, Model: record.Model, ReasoningEffort: record.ReasoningEffort, Alias: record.Alias, APIKey: MaskAPIKey(record.APIKey), APIKeyID: APIKeyIdentifier(record.APIKey), AuthType: record.AuthType, AuthIndex: strings.TrimSpace(record.AuthIndex), Source: MaskSensitiveSource(record.Source), LatencyNanos: nonNegativeDuration(record.Latency), TTFTNanos: nonNegativeDuration(record.TTFT), Failed: record.Failed, InputTokens: record.InputTokens, OutputTokens: record.OutputTokens, ReasoningTokens: record.ReasoningTokens, CachedTokens: record.CachedTokens, CacheReadTokens: record.CacheReadTokens, CacheCreationTokens: record.CacheCreationTokens, TotalTokens: record.TotalTokens, Cost: cost, PricedBy: pricedBy, Priced: priced}
}
