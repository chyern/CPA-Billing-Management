...
func (s *Store) HandleUsage(record UsageRecord) error {
	...
	event.Currency = s.state.Currency
	if value := strings.TrimSpace(record.Currency); value != "" {
		event.Currency = value
	}
	apiKey := s.nextAPIKeyAggregate(event)
	if !finite(apiKey.Cost) {
	...
	if err := s.persistUsageLocked(event, nextState, apiKey); err != nil {
		s.lastErr = err
		return err
	}
...
func usageEventFromRecord(record UsageRecord, cost float64, pricedBy string, priced bool) UsageEvent {
	return UsageEvent{RequestedAt: record.RequestedAt, Model: record.Model, ReasoningEffort: record.ReasoningEffort, Alias: record.Alias, APIKey: MaskAPIKey(record.APIKey), APIKeyID: APIKeyIdentifier(record.APIKey), AuthType: record.AuthType, AuthIndex: strings.TrimSpace(record.AuthIndex), Source: MaskSensitiveSource(record.Source), LatencyNanos: nonNegativeDuration(record.Latency), TTFTNanos: nonNegativeDuration(record.TTFT), Failed: record.Failed, InputTokens: record.InputTokens, OutputTokens: record.OutputTokens, ReasoningTokens: record.ReasoningTokens, CachedTokens: record.CachedTokens, CacheReadTokens: record.CacheReadTokens, CacheCreationTokens: record.CacheCreationTokens, TotalTokens: record.TotalTokens, Cost: cost, PricedBy: pricedBy, Priced: priced}
}
...
func aggregateKey(provider, model string) string { return strings.ToLower(strings.TrimSpace(model)) }
