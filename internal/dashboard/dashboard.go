...
<div class="header"><div><h1>CPA 费用统计</h1><div class="muted">按 CLIProxyAPI usage 事件计算，价格按每百万 token 计费</div></div><div class="toolbar"><span id="status" class="muted status"></span><button class="btn" id="refresh">刷新</button></div></div>
...
document.getElementById('refresh').onclick=async()=>{try{const res=await fetch(BASE+'?format=json',{credentials:'same-origin'});if(!res.ok)throw new Error(await res.text()||res.statusText);state=(await res.json()).summary;render();status('已刷新')}catch(e){status('刷新失败：'+e.message,true)}};render();
</script></body></html>`
